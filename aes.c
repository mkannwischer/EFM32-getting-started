...
int main(void)
{
  ...
  hal_aes128_ctr(buf, 67, iv, key);
  if(memcmp(buf, stream128, 67)) {
    hal_send_str("ERROR hal_aes128_ctr output does not match test vector.\n");
  } else {
    hal_send_str("OK hal_aes128_ctr.\n");
  }

  hal_aes256_ctr(buf, 67, iv, key);
  if(memcmp(buf, stream256, 67)) {
    hal_send_str("ERROR hal_aes256_ctr output does not match test vector.\n");
  } else {
    hal_send_str("OK hal_aes256_ctr.\n");
  }
  ...
  hal_send_str("benchmarking hal_aes128_ctr.");
  for(size_t i=1;i<=(1<<12);i<<=1){
    ...
    t0 = hal_get_time();
    hal_aes128_ctr(buf, i*16, iv, key);
    t1 = hal_get_time();
  ...
  hal_send_str("benchmarking hal_aes256_ctr.");
  for(size_t i=1;i<=(1<<12);i<<=1){
    ...
    t0 = hal_get_time();
    hal_aes256_ctr(buf, i*16, iv, key);
    t1 = hal_get_time();
